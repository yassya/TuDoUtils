...
'''
from TuDoUtils.plotInPlotHolder import plotInPlotHolder
...
def example():
    ...
    myUtils = plotInPlotHolder()
    
    #axis labels. I recommend setting them
    myUtils.xTitle = "some title for the x-axis"
    myUtils.yTitle = "something else for the y-axis"
    
    #this is how you configure the actual position and size of the pad
    # positions is the bottom left corner of the pad
    myUtils.middlePositions=(0.5,0.5)
    myUtils.middleSize=(0.4,0.4)
    ...
    myUtils.addPlot1D(myFirstPlot, "This will appear", "EY", False)
    myUtils.addPlot1D(mySecondPlot, "in the legend", "HIST", False)
    myUtils.addPlot1D(mySecondPlot, "in the legend", "HIST", True)
    myThirdPlot=mySecondPlot.Clone("dafq")
    myThirdPlot.Scale(1.5)
    myThirdPlot.SetFillColor(0)
    myThirdPlot.SetLineColor(ROOT.kBlue)
    myUtils.addPlot1D(myThirdPlot, "in the legend", "EY", True)
    ...
    myUtils.drawPlots(0.25,0.7, 0.8,0.8)
